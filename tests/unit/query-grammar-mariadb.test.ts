import { MariaDBQueryGrammar } from '../../src/query/grammars/MariaDBQueryGrammar';
...
function makeQb(table: string = 'posts'): { qb: QueryBuilder; grammar: MariaDBQueryGrammar } {
...
describe('MariaDBQueryGrammar', () => {
  // ── wrap / columnize ──────────────────────────────────────────────────────

  describe('wrap()', () => {
    it('wraps simple identifier with backticks', () => {
  ...
  describe('columnize()', () => {
    it('wraps and joins multiple columns with backticks', () => {
      const { grammar } = makeQb();
  ...
  describe('compileSelect()', () => {
    ...
    it('generates SELECT with explicit columns using backticks', () => {
      const { qb, grammar } = makeQb('posts');
      qb.select('id', 'title');
    ...
    it('generates column alias with AS using backticks', () => {
      const { qb, grammar } = makeQb('posts');
    ...
    it('generates INNER JOIN with backtick-quoted identifiers', () => {
      const { qb, grammar } = makeQb('posts');
    ...
    it('generates GROUP BY with backtick-quoted columns', () => {
      const { qb, grammar } = makeQb('posts');
      qb.groupBy('status');
      const { sql } = grammar.compileSelect(qb);
    ...
    it('generates FOR UPDATE lock hint', () => {
      const { qb, grammar } = makeQb('posts');
      qb.lockForUpdate();
      const { sql } = grammar.compileSelect(qb);
    ...
    it('generates LOCK IN SHARE MODE for shared lock', () => {
      const { qb, grammar } = makeQb('posts');
      qb.sharedLock();
      const { sql } = grammar.compileSelect(qb);
    ...
    it('generates complex SELECT with JOIN + WHERE + GROUP BY + HAVING + ORDER + LIMIT', () => {
      const { qb, grammar } = makeQb('posts');
  ...
  describe('WHERE variants', () => {
    ...
    it('whereColumn → both sides backtick-quoted', () => {
      const { qb, grammar } = makeQb('posts');
  ...
  describe('compileInsert()', () => {
    it('generates single-row INSERT with backtick-quoted identifiers and ? params', () => {
      const { qb, grammar } = makeQb('posts');
  ...
  // ── compileInsertGetId ─────────────────────────────────────────────────────
  // MariaDB 10.5+: RETURNING instead of MySQL's LAST_INSERT_ID()

  describe('compileInsertGetId()', () => {
    it('appends RETURNING `id` (default PK) to INSERT statement', () => {
      const { qb, grammar } = makeQb('posts');
      const { sql, bindings } = grammar.compileInsertGetId(qb, { title: 'Hello', status: 'draft' });
      expect(sql).toBe('INSERT INTO `posts` (`title`, `status`) VALUES (?, ?) RETURNING `id`');
      expect(bindings).toEqual(['Hello', 'draft']);
    });
    ...
    it('preserves all bindings from the INSERT portion', () => {
    ...
    it('produces RETURNING clause (unlike MySQL which uses LAST_INSERT_ID)', () => {
      const { qb, grammar } = makeQb('posts');
      const { sql } = grammar.compileInsertGetId(qb, { title: 'T' });
  ...
  describe('compileInsertOrIgnore()', () => {
    it('generates INSERT IGNORE INTO (inherited from MySQL grammar)', () => {
      const { qb, grammar } = makeQb('posts');
      const { sql, bindings } = grammar.compileInsertOrIgnore(qb, [{ title: 'Hello' }]);
      expect(sql).toBe('INSERT IGNORE INTO `posts` (`title`) VALUES (?)');
      expect(bindings).toEqual(['Hello']);
    });

    it('generates multi-row INSERT IGNORE', () => {
      const { qb, grammar } = makeQb('posts');
      const { sql } = grammar.compileInsertOrIgnore(qb, [{ slug: 'a' }, { slug: 'b' }]);
  ...
  describe('compileUpsert()', () => {
    it('generates INSERT ... ON DUPLICATE KEY UPDATE (inherited from MySQL)', () => {
      const { qb, grammar } = makeQb('posts');
    ...
    it('handles Expression in update columns without extra binding', () => {
      const { qb, grammar } = makeQb('posts');
      const values = [{ slug: 'hi', updated_at: new Expression('NOW()') }];
      const { sql, bindings } = grammar.compileUpsert(qb, values, ['slug'], ['updated_at']);
      expect(sql).toContain('`updated_at` = NOW()');
      expect(bindings).toEqual(['hi']);
    });
  });
  ...
  // MariaDB 10.5+ exclusive

  describe('compileUpdateReturning()', () => {
    it('appends RETURNING clause with single column', () => {
      const { qb, grammar } = makeQb('posts');
      qb.where('id', 1);
      const { sql, bindings } = grammar.compileUpdateReturning(qb, { title: 'Updated' }, ['id']);
      expect(sql).toContain('UPDATE `posts` SET `title` = ?');
      expect(sql).toContain('RETURNING `id`');
      expect(bindings).toEqual(['Updated', 1]);
    });

    it('appends RETURNING clause with multiple columns', () => {
      const { qb, grammar } = makeQb('users');
      qb.where('id', 5);
      const { sql } = grammar.compileUpdateReturning(qb, { name: 'Alice' }, ['id', 'name', 'email']);
      expect(sql).toContain('RETURNING `id`, `name`, `email`');
    });

    it('preserves WHERE bindings after UPDATE SET bindings', () => {
      const { qb, grammar } = makeQb('posts');
      qb.where('status', 'draft');
      const { sql, bindings } = grammar.compileUpdateReturning(
        qb,
        { status: 'published' },
        ['id']
      );
      expect(sql).toBe('UPDATE `posts` SET `status` = ? WHERE `status` = ? RETURNING `id`');
      expect(bindings).toEqual(['published', 'draft']);
    });
  });
  ...
  describe('compileDeleteReturning()', () => {
    it('appends RETURNING clause with single column to DELETE', () => {
      const { qb, grammar } = makeQb('posts');
      qb.where('id', 99);
      const { sql, bindings } = grammar.compileDeleteReturning(qb, ['id']);
      expect(sql).toBe('DELETE FROM `posts` WHERE `id` = ? RETURNING `id`');
    ...
    it('appends RETURNING clause with multiple columns', () => {
      ...
      const { sql } = grammar.compileDeleteReturning(qb, ['id', 'email']);
      expect(sql).toContain('RETURNING `id`, `email`');
    });

    it('works on DELETE FROM without WHERE', () => {
      const { qb, grammar } = makeQb('logs');
      const { sql, bindings } = grammar.compileDeleteReturning(qb, ['id']);
      expect(sql).toBe('DELETE FROM `logs` RETURNING `id`');
      expect(bindings).toEqual([]);
    });
  });
  ...
  describe('compileTruncate()', () => {
    it('generates TRUNCATE TABLE with backtick-quoted table name', () => {
      const { grammar } = makeQb();
  ...
  describe('compileAggregate()', () => {
    it('generates COUNT(*) with backtick aggregate alias', () => {
      const { qb, grammar } = makeQb('posts');
  ...
  describe('lock hints', () => {
    it('lockForUpdate appends FOR UPDATE at end of query', () => {
      const { qb, grammar } = makeQb('posts');
      qb.where('id', 1).lockForUpdate();
      const { sql } = grammar.compileSelect(qb);
      expect(sql).toContain('FOR UPDATE');
      expect(sql.trim().endsWith('FOR UPDATE')).toBe(true);
    });

    it('sharedLock appends LOCK IN SHARE MODE at end of query', () => {
      const { qb, grammar } = makeQb('posts');
      qb.where('id', 1).sharedLock();
      const { sql } = grammar.compileSelect(qb);
      expect(sql).toContain('LOCK IN SHARE MODE');
      expect(sql.trim().endsWith('LOCK IN SHARE MODE')).toBe(true);
    });
  });
});
