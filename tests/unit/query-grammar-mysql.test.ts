import { MySQLQueryGrammar } from '../../src/query/grammars/MySQLQueryGrammar';
...
function makeQb(table: string = 'posts'): { qb: QueryBuilder; grammar: MySQLQueryGrammar } {
...
describe('MySQLQueryGrammar', () => {
  ...
  describe('compileInsert()', () => {
    it('generates single-row INSERT with backtick-quoted identifiers', () => {
      const { qb, grammar } = makeQb('posts');
  ...
  describe('compileInsertGetId()', () => {
    it('generates plain INSERT (MySQL uses LAST_INSERT_ID, no RETURNING)', () => {
      const { qb, grammar } = makeQb('posts');
  ...
  describe('compileInsertOrIgnore()', () => {
    it('generates INSERT IGNORE INTO (MySQL syntax)', () => {
      const { qb, grammar } = makeQb('posts');
  ...
  describe('compileUpsert()', () => {
    it('generates INSERT ... ON DUPLICATE KEY UPDATE', () => {
      const { qb, grammar } = makeQb('posts');
    ...
    it('uniqueBy argument is ignored (MySQL uses ON DUPLICATE KEY)', () => {
      const { qb, grammar } = makeQb('posts');
      const values = [{ slug: 'hi', title: 'Hi' }];
      const { sql } = grammar.compileUpsert(qb, values, ['ignored_col'], ['title']);
      // MySQL does not include conflict columns in the SQL
    ...
    it('handles Expression in update columns without extra binding', () => {
      ...
      // No extra binding added for the expression
  ...
  describe('compileTruncate()', () => {
    it('generates TRUNCATE TABLE (MySQL supports real TRUNCATE)', () => {
      const { grammar } = makeQb();
    ...
    it('wraps the table name with backticks', () => {
      const { grammar } = makeQb();
      const { sql } = grammar.compileTruncate('my_table');
      expect(sql).toBe('TRUNCATE TABLE `my_table`');
    });

    it('does NOT generate DELETE FROM (unlike SQLite)', () => {
      const { grammar } = makeQb();
