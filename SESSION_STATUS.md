...
| CI — npm test step | ✅ |
| Stack: Vitest v2 + @vitest/coverage-v8 | ✅ |

### ⏳ Próxima sessão — Aumentar Cobertura de Testes

**Cobertura atual (2026-06-07):** 30.71% statements / 25.92% funções / 69.77% branches
**Meta:** ≥ 70% statements, ≥ 60% funções

---

## 📋 PLANO DE TESTES — EXECUÇÃO PARALELA

> **Instrução para a sessão:** Spawne os 6 agentes abaixo em paralelo (todos de uma vez com Agent tool).
> Cada agente é independente — conexão SQLite nomeada diferente, sem arquivos compartilhados.
> Ao final, rode `npm test` e `npm run test:coverage` para validar.

---

### Grupo A — Unit Tests (sem banco, puro TypeScript)

#### Agente A1 — `tests/unit/query-builder-advanced.test.ts`
Cobertura alvo: `src/query/QueryBuilder.ts` (atual 64% → meta 85%)
Testa métodos ainda não cobertos:
- `update({ col: val })` → verifica SQL `UPDATE ... SET ... WHERE`
- `delete()` → verifica SQL `DELETE FROM ... WHERE`
- `increment('views', 2)` / `decrement`
- `count()`, `sum('price')`, `avg('score')`, `min('id')`, `max('id')`
- `truncate()`
- `insertOrIgnore([...])`
- `upsert([...], uniqueBy, updateCols)` → verifica ON CONFLICT SQL
- `whereBetween('age', [18, 65])` / `whereNotBetween`
- `whereColumn('updated_at', '>', 'created_at')`
- `forPage(2, 15)` → limit 15 offset 15
- `when(true, cb)` / `when(false, cb)`
- `tap(cb)`
- `orderByRaw('RANDOM()')`
- `selectRaw('COUNT(*) as total')`
- `having('total', '>', 10)` / `havingRaw`
- `lockForUpdate()` / `sharedLock()`
- `toSql()` retorna SQL sem executar

**Como instanciar QueryBuilder sem banco:**
```ts
import { QueryBuilder } from '../../src/query/QueryBuilder';
import { PostgresQueryGrammar } from '../../src/query/grammars/PostgresQueryGrammar';
const conn = { query: vi.fn().mockResolvedValue({ rows: [], rowCount: 0, fields: [] }),
               transaction: vi.fn(), disconnect: vi.fn().mockResolvedValue(undefined),
...
#### Agente A2 — `tests/unit/query-grammar-sqlite.test.ts` + `tests/unit/query-grammar-mysql.test.ts`
Cobertura alvo: `src/query/grammars/SQLiteQueryGrammar.ts` e `MySQLQueryGrammar.ts` (atual ~10-58% → meta 80%)

**SQLiteQueryGrammar** — testa todos os métodos `compile*`:
- `compileSelect` com JOIN, WHERE, GROUP BY, HAVING, ORDER, LIMIT, OFFSET
- `compileInsert` com múltiplas linhas
- `compileInsertGetId` → usa `compileInsert` (sem RETURNING)
- `compileInsertOrIgnore` → `INSERT OR IGNORE INTO`
- `compileUpsert` → `INSERT INTO ... ON CONFLICT (...) DO UPDATE SET`
- `compileUpdate` com WHERE
- `compileDelete` com WHERE
- `compileTruncate` → `DELETE FROM` (SQLite não tem TRUNCATE)
- `compileWhereNull` / `compileWhereNotNull`
- `compileWhereBetween`
- Identificadores entre aspas duplas: `"table"."column"`
- Parâmetros posicionais `?`

**MySQLQueryGrammar** — idem mas:
- Identificadores entre backticks: `` `table`.`column` ``
- Parâmetros posicionais `?`
- `compileInsertGetId` → `INSERT INTO ... (cols) VALUES (?) -- lastInsertId`
- `compileUpsert` → `INSERT INTO ... ON DUPLICATE KEY UPDATE`
- `compileLock` → `FOR UPDATE` / `LOCK IN SHARE MODE`

**Como instanciar:**
```ts
import { SQLiteQueryGrammar } from '../../src/query/grammars/SQLiteQueryGrammar';
import { QueryBuilder } from '../../src/query/QueryBuilder';
const conn = { query: vi.fn(), transaction: vi.fn(), disconnect: vi.fn(), isConnected: vi.fn(), getGrammar: vi.fn() };
const grammar = new SQLiteQueryGrammar();
const qb = new QueryBuilder(conn as any, grammar);
qb.from('posts');
const { sql, bindings } = grammar.compileSelect(qb);
```

#### Agente A3 — `tests/unit/model-builder.test.ts`
Cobertura alvo: `src/model/ModelBuilder.ts` (atual 27% → meta 55%)
Testa métodos de ModelBuilder que não precisam de banco real — apenas verificam SQL gerado ou comportamento do builder:
- `where`, `orWhere`, `whereIn`, `whereNull`, `whereNotNull`
- `orderBy`, `orderByDesc`, `latest`, `oldest`
- `select`, `limit`, `offset`, `forPage`
- `with('relation')` armazena eager load map
- `withCount('relation')` armazena eager load
- `withoutGlobalScope('name')` remove scope
- `scopes` chain (local scope via Proxy)
- `ModelBuilder.toSql()` retorna SQL sem executar
...
@table({ name: 'test_items', timestamps: false, connection: 'mock' })
class TestItem extends Model {}

// Mock connection
ConnectionManager.addConnection('mock', { driver: 'sqlite', filename: ':memory:' });
const builder = TestItem.query();
```

---

### Grupo B — Integration Tests (SQLite :memory:)

> Cada arquivo usa uma conexão nomeada diferente para rodar em paralelo sem conflito.

#### Agente B1 — `tests/integration/model-relations-morph.test.ts`
Conexão: `'morph'`
Cobertura alvo: `MorphOne`, `MorphMany`, `MorphTo`, `MorphToMany`, `MorphedByMany` (atual 10-19% → meta 60%)

**Schema:**
```ts
// posts: id, title, type='post'
// videos: id, title, type='video'
...
**Testes:**
- `post.comments()` → `MorphMany` → retorna só comments do post
- `comment.commentable()` → `MorphTo` → retorna o post ou video correto
- `video.comments()` → `MorphMany` → retorna só comments do video
- `post.tags()` → `MorphToMany` → retorna tags via pivot polimórfico
- `tag.posts()` → `MorphedByMany` → retorna posts associados a essa tag
- Eager loading: `Post.with('comments').get()`
- MorphMap: `MorphMap.register({ post: Post, video: Video })` + resolve
...
class MorphPost extends Model {
  ...
  tags(): MorphToMany<MorphTag> {
    return this.morphToMany(MorphTag, 'morph_taggables', 'taggable', 'tag_id', 'id', 'id');
  }
}
// etc.
```

#### Agente B2 — `tests/integration/model-advanced.test.ts`
Conexão: `'advanced'`
Cobertura alvo: `Model.ts` métodos avançados (atual 50% → meta 70%), `ModelBuilder` métodos avançados, `Paginator`

**Schema:** `adv_users: id, name, email, age, status, created_at, updated_at`

**Testes:**
- `Model.firstOrCreate({ email }, { name })` — find existing
- `Model.firstOrCreate({ email }, { name })` — create new
- `Model.updateOrCreate({ email }, { name })` — update existing
- `Model.updateOrCreate({ email }, { name })` — create new
- `Model.firstOrNew({ email })` — returns unsaved instance
- `User.paginate(2)` → `Paginator` com `currentPage`, `lastPage`, `total`, `data`
- `User.simplePaginate(2)` → `SimplePaginator`
- `User.count()` / `User.sum('age')` / `User.avg('age')` / `User.max('age')` / `User.min('age')`
- `User.increment('age', 1)` / `User.decrement('age', 1)`
- `User.query().chunk(2, cb)` → processa em lotes
- `User.query().each(cb)` → itera sobre todos
- `User.query().lazy()` → async generator
- `User.withoutTimestamps(async () => { ... })` → não seta timestamps
- `Model.preventSilentlyDiscardingAttributes()` → lança em assign não-fillable
- `model.replicate()` → cria cópia sem id
- `model.is(other)` / `model.isNot(other)`
- `model.wasRecentlyCreated` = true após create
- `User.query().whereBetween('age', [18, 30]).get()`
- `User.query().whereIn('status', ['active','pending']).get()`
- `User.query().latest('created_at').first()`
- `User.query().when(true, q => q.where('status', 'active')).get()`
- `User.all()` → Collection com `first()`, `last()`, `filter()`, `map()`, `pluck()`

#### Agente B3 — `tests/integration/model-events.test.ts`
Conexão: `'events'`
Cobertura alvo: `src/model/events/` (atual 38% → meta 80%), `src/model/decorators/observe.ts`

**Schema:** `evt_users: id, name, email`

**Testes:**
- `User.creating(cb)` / `User.created(cb)` — listener registrado via static
- `User.updating(cb)` / `User.updated(cb)`
- `User.deleting(cb)` / `User.deleted(cb)`
- `User.saving(cb)` / `User.saved(cb)`
- Retornar `false` do `creating` aborta o insert
- `User.withoutEvents(async () => { ... })` — não dispara eventos
- `withoutEvents` helper importado de EventDispatcher
- `@observedBy(UserObserver)` → Observer com métodos `creating`, `created`, etc.
- `Model.observe(UserObserver)` → registra observer programaticamente
- Eventos disparados na ordem correta: `saving → creating → created → saved`
- `model.getEventDispatcher()` existe

#### Agente B4 — `tests/integration/model-factory-resources.test.ts`
Conexão: `'factory'`
Cobertura alvo: `src/factory/` (atual 0% → meta 70%), `src/resources/` (atual 0% → meta 60%)

**Schema:** `fac_users: id, name, email, age`

**Factory:**
```ts
class UserFactory extends Factory<FacUser> {
  protected model = FacUser;
  definition() {
...
**Testes Factory:**
- `UserFactory.new().make()` → instância não salva
- `UserFactory.new().create()` → salva no banco
- `UserFactory.new().count(3).create()` → cria 3 registros
- `UserFactory.new().state({ age: 99 })` → override atributos
- `UserFactory.new().sequence({ name: 'User 1' }, { name: 'User 2' })` → alterna
- `Sequence` iterável
- `UserFactory.new().afterMaking(cb)` → callback após make
- `UserFactory.new().afterCreating(cb)` → callback após create
- `UserFactory.new().has(PostFactory.new().count(2), 'posts')` → cria com relação

**Testes Resources:**
```ts
class UserResource extends Resource<FacUser> {
  toArray() {
    return { id: this.resource._attributes.id, name: this.resource._attributes.name };
  }
}
```
- `UserResource.make(user).resolve()` → objeto serializado
- `UserResource.collection([u1, u2]).resolve()` → array de objetos
- `resource.when(true, () => ({ extra: 'yes' }))` → inclui campo condicional
- `resource.when(false, () => ({ extra: 'yes' }))` → omite campo
- `resource.whenLoaded('posts', () => ...)` → inclui se relação carregada
- `resource.mergeWhen(true, { meta: 1 })` → merge condicional

---

### Agente B5 — `tests/integration/model-collection.test.ts`
Conexão: `'collection'`
Cobertura alvo: `src/model/Collection.ts` (atual 33% → meta 75%), `src/model/ModelCollection.ts` (atual 18% → meta 60%)

**Schema:** `col_users: id, name, role, score`

**Collection tests:**
- `collection.first()` / `collection.last()`
- `collection.filter(fn)` → nova collection filtrada
- `collection.map(fn)` → array mapeado
- `collection.pluck('name')` → array de valores
- `collection.toArray()` → array JS
- `collection.isEmpty()` / `collection.isNotEmpty()`
- `collection.count()` → número de itens
- `collection.contains(item)` → verifica existência
- `collection.each(fn)` → itera
- `collection.sum('score')` / `collection.avg('score')` / `collection.min('score')` / `collection.max('score')`
- `collection.sortBy('name')` / `collection.sortByDesc('score')`
- `collection.groupBy('role')` → Map por role
- `collection.keyBy('id')` → Map por id
- `collection.unique('role')` → sem duplicatas
- `collection.chunk(2)` → array de collections
- `collection.zip([1, 2, 3])` → pares
- `collection.toJson()` → string JSON

**ModelCollection tests (usa `ModelCollection<User>`):**
- `mc.findByKey(id)` / `mc.findOrFail(id)`
- `mc.modelKeys()` → array de ids
- `mc.except([id1])` → sem esses ids
- `mc.only([id1, id2])` → só esses ids
- `mc.diff(otherMc)` → diferença
- `mc.intersect(otherMc)` → interseção
- `mc.toQuery()` → builder com WHERE id IN (...)
- `mc.fresh()` → recarrega do banco
- `mc.makeVisible(['email'])` / `mc.makeHidden(['email'])`

---

### Configuração paralela (vitest.config.ts já existente)

O `vitest.config.ts` atual já usa `globals: true` e `include: ['tests/**/*.test.ts']`.
Para rodar em paralelo eficientemente, adicionar ao `vitest.config.ts`:
```ts
test: {
  globals: true,
  environment: 'node',
  include: ['tests/**/*.test.ts'],
  pool: 'forks',          // cada arquivo em processo separado = SQLite :memory: isolada
  poolOptions: {
...
Cada arquivo de integração deve usar seu próprio `Schema.useGrammar()`:
...
const CONN = 'nome_unico_da_conexao'; // diferente para cada arquivo

beforeAll(async () => {
  ConnectionManager.addConnection(CONN, { driver: 'sqlite', filename: ':memory:' });
  // IMPORTANTE: useGrammar é global — definir aqui garante SQLite grammar para este arquivo
  Schema.useGrammar(new SQLiteSchemaGrammar());
  await Schema.create('tabela', (t) => { ... }, CONN);
});

afterAll(async () => {
  await ConnectionManager.getConnection(CONN).disconnect();
  // NÃO usar disconnectAll() — mata conexões de outros arquivos rodando em paralelo
});

beforeEach(async () => {
  await ConnectionManager.getConnection(CONN).query('DELETE FROM tabela');
});
```

> ⚠️ **ATENÇÃO:** `Schema.useGrammar()` é estado global compartilhado. Com `pool: 'forks'`,
> cada arquivo roda em processo isolado então não há conflito. Sem forks, pode haver race condition.

---

### Meta de cobertura por arquivo após os novos testes

| Arquivo | Antes | Meta |
|---------|-------|------|
| `QueryBuilder.ts` | 64% | 85% |
| `SQLiteQueryGrammar.ts` | 58% | 85% |
| `MySQLQueryGrammar.ts` | 10% | 75% |
| `Model.ts` | 50% | 72% |
| `ModelBuilder.ts` | 27% | 55% |
| `Collection.ts` | 33% | 75% |
| `ModelCollection.ts` | 18% | 60% |
| `SoftDeletes.ts` | 94% | 95% |
| `HasMany.ts` | 43% | 70% |
| `HasOne.ts` | 34% | 65% |
| `BelongsTo.ts` | 35% | 65% |
| `MorphMany.ts` | 15% | 60% |
| `MorphOne.ts` | 19% | 60% |
| `MorphTo.ts` | 17% | 55% |
| `MorphToMany.ts` | 13% | 55% |
| `EventDispatcher.ts` | 38% | 80% |
| `Factory.ts` | 0% | 70% |
| `Resource.ts` | 0% | 60% |
| **Total** | **30%** | **≥ 65%** |
