...
/**
 * Compiles `QueryBuilder` state into SQLite-flavoured SQL.
 *
 * SQLite uses positional `?` parameters and double-quote identifier quoting.
 * Notable limitations vs Postgres/MySQL:
 * - No RETURNING clause (insertGetId reads lastInsertRowid from the adapter)
 * - No native UPSERT before SQLite 3.24 — uses INSERT OR REPLACE / ON CONFLICT
 * - TRUNCATE not supported — compiled as DELETE FROM + reset sequence
 * - FOR UPDATE / locking hints are silently dropped (SQLite uses file-level locking)
 */
export class SQLiteQueryGrammar implements QueryGrammar {
  // ── SELECT ────────────────────────────────────────────────────────────────
  ...
  compileSelect(builder: QueryBuilder): CompiledQuery {
    ...
    // SQLite does not support locking hints — omit silently
  ...
  compileInsertGetId(builder: QueryBuilder, values: Record<string, unknown>): CompiledQuery {
    // SQLite: read lastInsertRowid from adapter — no RETURNING needed
  ...
  compileInsertOrIgnore(builder: QueryBuilder, values: Record<string, unknown>[]): CompiledQuery {
    ...
    return {
      sql: compiled.sql.replace(/^INSERT INTO/, 'INSERT OR IGNORE INTO'),
  ...
  compileTruncate(table: string): CompiledQuery {
    // SQLite has no TRUNCATE — DELETE + reset autoincrement sequence
    return {
