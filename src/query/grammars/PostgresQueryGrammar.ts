...
/**
 * Compiles `QueryBuilder` state into PostgreSQL-flavoured SQL.
 *
 * Postgres uses positional parameters (`$1`, `$2` …) rather than `?`.
 * A `BindingCollector` is threaded through every compiler method so the
 * final binding index is always consistent across clauses.
 */
export class PostgresQueryGrammar implements QueryGrammar {
  // ── SELECT ────────────────────────────────────────────────────────────────
  ...
  compileSelect(builder: QueryBuilder): CompiledQuery {
    ...
    parts.push(this.compileColumns(builder, b));
    parts.push(`FROM ${this.compileFrom(builder, b)}`);
    ...
    if (builder.lockMode) parts.push(builder.lockMode);
  ...
  compileInsertGetId(builder: QueryBuilder, values: Record<string, unknown>): CompiledQuery {
    ...
    return {
      sql: `${compiled.sql} RETURNING ${this.wrap(builder.primaryKey ?? 'id')}`,
  ...
  compileUpsert(
    ...
    const compiled = this.compileInsert(builder, values);
    const conflictCols = uniqueBy.map((c) => this.wrap(c)).join(', ');
    const updateSql = updateColumns
      .map((c) => `${this.wrap(c)} = EXCLUDED.${this.wrap(c)}`)
      .join(', ');

    return {
      sql: `${compiled.sql} ON CONFLICT (${conflictCols}) DO UPDATE SET ${updateSql}`,
      bindings: compiled.bindings,
    };
  }
  ...
  compileTruncate(table: string): CompiledQuery {
    return { sql: `TRUNCATE TABLE ${this.wrap(table)} RESTART IDENTITY CASCADE`, bindings: [] };
  }
  ...
  private compileColumns(builder: QueryBuilder, _b: BindingCollector): string {
  ...
  private compileFrom(builder: QueryBuilder, _b: BindingCollector): string {
    if (!builder.fromTable) throw new Error('[orion] QueryBuilder: no table specified.');

    if (builder.fromTable instanceof Expression) {
      return builder.fromTable.getValue();
    }
  ...
  private compileWhere(w: WhereClause, b: BindingCollector): string {
    switch (w.type) {
      ...
      case 'exists': {
        const sub = w.subQuery!;
        const compiled = this.compileSelect(sub);
  ...
  wrap(value: string | Expression): string {
    ...
    if (value === '*') return '*';
    if (value.includes('(')) return value; // function call, leave as-is
    if (value.includes('.')) {
      return value
        .split('.')
